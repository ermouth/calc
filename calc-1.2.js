/*
 * ermouth.calc interpolator 1.2
 * 
 * (c) ermouth
 * 
 * Requires SugarJS 1.2.5+
 * 
 */
...
if (!calc) var calc = {
	...
	//will return function of x,y, calculating rounded integer product of arguments if they both are >0 or null otherwise
		...
		if ($.type(data)!="array" || !data.length) return null;
		
		var a = data.slice(0), xc = a[0].length, yc = a.length, i, r, ok;
		var ax = a[0].slice(1).join("ᴥ").split("ᴥ"), ay = a.map(function(e){return e[0]}).slice(1).join("ᴥ").split("ᴥ");
		...
		ok = (Object.isFunction(bounds))?(bounds):function(x,y) {	
		...
		var f = function (x0,y0) {
			var x = xs?String(x0):(Number(x0)||0), y = ys?String(y0):(Number(y0)||0), r ,i, j, n, ai=[];
	...
	tiles: function (x0,y0,px,py, gap) {
		...
		var add = add0||0, x=1*x0+add, y=1*y0+add;
	...
	report:function (order, data, precision) {
		...
		if ($.type(order)=="string") rows=order.lines().compact();
		if ($.type(rows)!="array" || !rows.length || $.type(data)!="object") return {total:null, rows:[], errors:["Incorrect params"]};
		
		
		//counting subtotal
		for (i=0; i<rows.length; i++) {
			ri = rows[i], rt = $.type(ri), row = null;
			if (rt=="string") {ri = ri.compact().split(" "); rt="array"}
			//traverse array
			if (rt=="array" && ri.length>1) {	
				...
				if (/^nu/.test($.type(r_))) { //null or number
					if (!isNaN(r_) && r_!==null) {
						if (/^%/.test(rc)) {} 
		...
		for (i=0; i<rows.length; i++) {
			...
			if (rt=="array" && ri.length>1) {	
				...
				if (/^nu/.test($.type(r_)) && !isNaN(r_) && r_!==null && /^%/.test(rc)) backp += Number(r_) 
			}
		}
		...
		for (i=0; i<rows.length; i++) {
			...
			if (rt=="array" && ri.length>1) {	
				...
				if (/^nu/.test($.type(r_))) { //null or number
					...
					if (r_!==undefined) row.v = String(r_);
